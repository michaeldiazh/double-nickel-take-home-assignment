 ...
 */
const generatePlaceholderArray = (count, offset) => {
    const placeholders = [];
 ...
 */
const buildComparisonOperator = (columnRef, filter, placeholder) => {
 ...
 */
const buildInOperator = (columnRef, filter, placeholder) => {
 ...
 */
const buildLikeOperator = (columnRef, filter, placeholder) => {
    if (placeholder === undefined) {
        throw new Error(`Parameter placeholder is required for filter operator: ${filter}`);
    }
    return `${columnRef} ${filter} ${placeholder}`;
 ...
 */
const buildIsNullOperator = (columnRef, filter) => {
 ...
 */
const buildWhereCondition = (whereOption, placeholderOffset) => {
 ...
 */
const generatePlaceholders = (filter, value, placeholderOffset) => {
 ...
 */
const buildConditionsAndValues = (whereOptions) => {
    const values = [];
    const conditions = [];
 ...
 *
 */
const buildWhereCompositionClause = (whereCompositionOption) => {
...
const buildWhereClause = (whereClause) => {
    ...
    if ((0, common_1.isWhereCompositionOption)(whereClause)) {
