...
import type { ConversationRequirementValue } from './types';
import { RequirementStatus } from '../../entities/enums';
import {z} from "zod";

/**
 * Result of parsing an LLM response for a requirement value.
 * Includes the parsed value, LLM's assessment, and confidence level.
 */
export interface RequirementParseResult<T extends ConversationRequirementValue> {
  /**
...
/**
 * @deprecated Use RequirementParseResult instead
 */
export type ParseResult<T extends ConversationRequirementValue> = RequirementParseResult<T>;
